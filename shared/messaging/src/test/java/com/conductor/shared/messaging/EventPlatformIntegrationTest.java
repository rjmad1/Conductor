...
class EventPlatformIntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        publisher = new EventPublisher(connectionManager, schemaValidator, auditLogger, observability, "test-service");
        ...
        if (conn != null) {
            try {
                ...
                StreamConfiguration tenantConfig = StreamConfiguration.builder()
                        .name("TENANT_STREAM")
                ...
                System.out.println("Warning: JetStream management failed, NATS might be starting or not in JetStream mode: " + e.getMessage());
            }
        }
    }
    ...
    void testPublishAndSubscribeSuccess() throws Exception {
        ...
        // We wrote tenant.profile.created.v1.json earlier, so let's publish using domain "tenant", entity "profile", action "created".
        String payload = "{\"id\":\"" + tenantId + "\",\"name\":\"Test Tenant\",\"domain\":\"example.com\"}";
    ...
    void testTenantIsolationEnforced() throws Exception {
        ...
        Subscription subA = consumer.subscribe(
                "tenant-a-group",
                "tenant",
                "profile",
                "created",
