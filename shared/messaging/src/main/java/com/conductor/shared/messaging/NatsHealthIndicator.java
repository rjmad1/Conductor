...
/**
 * Exposes NATS connection status to Spring Boot Actuator health endpoint. Reports DOWN when the
 * connection is null (startup failure) or not CONNECTED. Callers should check /actuator/health for
 * readiness before sending traffic.
 */
@Component
@ConditionalOnEnabledHealthIndicator("nats")
public class NatsHealthIndicator implements HealthIndicator {
  ...
  public Health health() {
    ...
    return Health.down().withDetail("status", status.name()).build();
  }
}
