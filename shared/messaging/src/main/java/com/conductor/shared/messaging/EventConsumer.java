...
public class EventConsumer {
    ...
    private final java.util.concurrent.ConcurrentHashMap<Subscription, Dispatcher> subscriptionDispatchers = 
    ...
    public <T> Subscription subscribe(
            ...
            String entity,
            String action,
            String tenantScopeId, // Nullable. If present, restricts subscription & visibility to this tenant
        ...
        String durableName = String.format("%s_%s_%s_%s", 
                tenantScopeId != null ? "tenant_" + tenantScopeId : "global", 
        ...
        try {
            ...
            // Dispatcher callback
            MessageHandler messageHandler = (Message msg) -> {
                ...
                try {
                    ...
                    ConductorEvent<String> rawEvent = objectMapper.readValue(envelopeJson, 
                            new TypeReference<ConductorEvent<String>>() {});
                    ...
                    if (tenantScopeId != null && !tenantScopeId.equals(rawEvent.getTenantId())) {
                        log.error("SECURITY VIOLATION: Cross-tenant event leakage blocked. " +
                                "Consumer tenantId {} received event tenantId {} on subject {}", 
                                tenantScopeId, rawEvent.getTenantId(), msg.getSubject());
                        auditLogger.logEvent("TENANT_VIOLATION", "EVENT_CONSUME:" + rawEvent.getEventId(),
                                "FAILURE", String.format("Leakage blocked: consumer=%s, event=%s", 
                    ...
                    boolean isValid = schemaValidator.validate(domain, entity, action, 
                            rawEvent.getSchemaVersion(), rawEvent.getPayload());

                    if (!isValid) {
                        log.warn("Poison message detected (Schema validation failed). Routing to DLQ: {}", rawEvent.getEventId());
                    ...
                    ConductorEvent<T> typedEvent = ConductorEvent.<T>builder()
                    ...
                    try {
                        ...
                        log.error("Error executing handler for event: {}", rawEvent.getEventId(), handlerException);
                    ...
                    observability.recordConsumption(domain, entity, action, activeTenantId, duration, success);
                    MDC.clear();
                }
            };
    ...
    private void handleProcessingFailure(Message msg, ConductorEvent<String> event, Exception ex, String tenantId) {
        ...
        if (deliveries >= 5) {
            ...
            log.warn("Retrying message {} (Attempt {}). Backoff delay: {}s", event.getEventId(), deliveries, backoffSeconds);
            msg.nakWithDelay(Duration.ofSeconds(backoffSeconds));
        }
    }

    private void routeToDlq(Message msg, ConductorEvent<String> event, String reason, String tenantId) {
        try {
            Connection conn = connectionManager.getConnection();
            if (conn == null) {
            ...
            String originPayload = event != null ? objectMapper.writeValueAsString(event) : new String(msg.getData(), StandardCharsets.UTF_8);
            String dlqWrapper = String.format(
            ...
            log.info("Successfully routed poison message {} to DLQ subject: {}", 
                    event != null ? event.getEventId() : "unparseable", dlqSubject);
            
            auditLogger.logEvent("DLQ_ESCALATION", "EVENT:" + (event != null ? event.getEventId() : "unparseable"),
                    "SUCCESS", "Event routed to DLQ. Subject: " + dlqSubject + ", Reason: " + reason);
