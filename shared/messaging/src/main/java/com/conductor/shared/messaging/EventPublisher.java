...
public class EventPublisher {
    ...
    public <T> boolean publish(String domain, String entity, String action, String schemaVersion, T payload) {
        ...
        try {
            ...
            boolean isValid = schemaValidator.validate(domain, entity, action, schemaVersion, payloadJson);
            if (!isValid) {
                log.error("Schema validation failed for event publication on subject {}. Blocking publication.", subject);
                auditLogger.logEvent("PUBLISH_REJECTED", "SCHEMA:" + domain + "." + entity + "." + action, 
                        "FAILURE", "Schema validation failed for payload: " + payloadJson);
                return false;
            }
            ...
            ConductorEvent<String> event = ConductorEvent.<String>builder()
            ...
            log.info("Successfully published event to subject {} (Stream: {}, Seq: {})", 
