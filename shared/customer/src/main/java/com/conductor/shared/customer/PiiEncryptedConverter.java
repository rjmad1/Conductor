...
/**
 * JPA AttributeConverter that encrypts/decrypts Tier-1 PII fields (phone, email, name)
 * using AES-256-GCM before storing to PostgreSQL.
 *
 * Security compliance: SG-006 — Customer PII must be encrypted at rest.
 * Key source: ${CUSTOMER_PII_ENCRYPTION_KEY} environment variable (32-byte Base64 encoded).
 *
 * Ciphertext format: Base64(IV[12] || TAG[16] || CIPHERTEXT)
 * IV is randomly generated per encryption. Authentication tag is included (GCM provides AEAD).
 *
 * ponytail: Env-var key is acceptable for MVP. Upgrade path: replace key loading with
 *           AWS KMS `GenerateDataKey` call and cache the DEK per application lifecycle.
 */
@Converter
public class PiiEncryptedConverter implements AttributeConverter<String, String> {
    ...
    public PiiEncryptedConverter() {
        ...
        if (keyBytes.length != 32) {
            throw new IllegalStateException(
                "'" + ENV_KEY_NAME + "' must decode to exactly 32 bytes (256 bits). " +
