...
public class SchemaValidator {
  ...
  public boolean validate(
      String domain, String entity, String action, String schemaVersion, String payloadJson) {
    try {
      String schemaFileName =
          String.format("%s.%s.%s.%s.json", domain, entity, action, schemaVersion);
      ...
      if (Files.exists(path)) {
        ...
        if (schemaStream == null) {
          log.error(
              "Schema file not found at: {} or on classpath: schemas/{}",
              path.toAbsolutePath(),
              schemaFileName);
      ...
      if (!assertions.isEmpty()) {
        log.warn(
            "Schema validation failed for event {}.{}.{} (version {}). Errors: {}",
            domain,
            entity,
            action,
            schemaVersion,
      ...
      log.error(
          "Error validating event payload against schema: {}.{}.{}", domain, entity, action, e);
      return false;
    }
  }
}
