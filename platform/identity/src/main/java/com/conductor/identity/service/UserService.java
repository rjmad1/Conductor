...
public class UserService {
    ...
    public User createUser(String email, String password) {
        ...
        eventPublisher.publishEvent("identity", "user", "created", 
                String.format("{\"id\":\"%s\",\"email\":\"%s\",\"tenantId\":\"%s\"}", savedUser.getId(), email, tenantId));
        
        auditLogger.logEvent("CREATE", "USER:" + savedUser.getId(), "SUCCESS", "User created and default membership assigned");
    ...
    public User inviteUser(String email, String role) {
        ...
        eventPublisher.publishEvent("identity", "user", "invited", 
                String.format("{\"id\":\"%s\",\"email\":\"%s\",\"role\":\"%s\"}", user.getId(), email, role));
        
        auditLogger.logEvent("INVITE", "USER:" + user.getId(), "SUCCESS", "User invited with role: " + role);
    ...
    public void assignRole(UUID userId, String role) {
        ...
        eventPublisher.publishEvent("identity", "role", "assigned", 
    ...
    public Optional<APIKey> validateApiKey(String plaintextKey) {
        ...
        // Verify key matching
        Optional<APIKey> apiKeyOpt = apiKeyRepository.findAll().stream()
