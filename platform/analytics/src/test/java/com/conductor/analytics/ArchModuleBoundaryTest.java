...
/**
 * ArchUnit test verifying the analytics module respects modular monolith boundaries.
 * The analytics domain must NOT import from other platform domain packages.
 */
class ArchModuleBoundaryTest {
    ...
    void analyticsDoesNotImportWorkflow() {
        noClasses().that().resideInAPackage("com.conductor.analytics..")
    ...
    void analyticsDoesNotImportCustomer() {
        noClasses().that().resideInAPackage("com.conductor.analytics..")
                .should().dependOnClassesThat().resideInAPackage("com.conductor.customer..")
                .check(classes);
    }
    ...
    void analyticsDoesNotImportMessaging() {
        noClasses().that().resideInAPackage("com.conductor.analytics..")
                .should().dependOnClassesThat().resideInAPackage("com.conductor.messaging..")
                .check(classes);
    }
    ...
    void analyticsDoesNotImportTenant() {
        noClasses().that().resideInAPackage("com.conductor.analytics..")
                .should().dependOnClassesThat().resideInAPackage("com.conductor.tenant..")
                .check(classes);
    }
}
