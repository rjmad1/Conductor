...
/**
 * Tenant isolation verification tests for analytics queries.
 * Validates that all ClickHouse queries include tenant_id filtering
 * and that cross-tenant data leakage is structurally prevented.
 */
class TenantIsolationTest {

    /**
     * Verify that all ClickHouse SQL scripts include tenant_id in partition keys.
     * This is a structural check — actual data isolation is enforced by:
     * 1. ClickHouse table partitioning by tenant_id
     * 2. All metrics service queries include tenant_id as first parameter
     * 3. NATS event ingestion preserves tenant_id from source events
     */
    @Test
