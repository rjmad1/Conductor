...
public class KpiEngine {
    ...
    public void scheduledEvaluation() {
        ...
        for (KpiValue kpi : results) {
            if (kpi.getStatus() != KpiValue.KpiStatus.HEALTHY) {
                log.warn("KPI alert: {} = {} (status: {})", kpi.getKpiName(), kpi.getComputedValue(), kpi.getStatus());
            }
        }
    ...
    double computeMetricValue(KpiDefinition def) {
        ...
        String sql = buildMetricQuery(metricName, def.getTenantId() != null ? def.getTenantId().toString() : null);
        if (sql == null) {
    ...
    private String buildMetricQuery(String metricName, String tenantId) {
        ...
        return switch (metricName) {
            case "workflow_success_rate" ->
                    "SELECT sumIf(executions_count, status = 'completed') * 100.0 / greatest(sum(executions_count), 1) " +
                    "FROM workflow_metrics_hourly WHERE hour >= ? AND hour <= ?" + tenantClause;
            case "message_delivery_rate" ->
                    "SELECT sumIf(message_count, action = 'delivered') * 100.0 / greatest(sumIf(message_count, action = 'dispatched'), 1) " +
                    "FROM messaging_metrics_hourly WHERE hour >= ? AND hour <= ?" + tenantClause;
            case "integration_success_rate" ->
                    "SELECT sumIf(event_count, action != 'failed') * 100.0 / greatest(sum(event_count), 1) " +
                    "FROM integration_metrics_hourly WHERE hour >= ? AND hour <= ?" + tenantClause;
            case "active_tenants" ->
                    "SELECT count(DISTINCT tenant_id) FROM tenant_metrics_daily WHERE day >= toDate(?) AND day <= toDate(?)" + tenantClause;
            case "customer_growth" ->
                    "SELECT sum(customer_count) FROM customer_metrics_daily WHERE action = 'created' AND day >= toDate(?) AND day <= toDate(?)" + tenantClause;
            case "active_customers" ->
                    "SELECT count(DISTINCT tenant_id) FROM customer_metrics_daily WHERE day >= toDate(?) AND day <= toDate(?)" + tenantClause;
            case "response_rate" ->
                    "SELECT sumIf(message_count, action = 'read') * 100.0 / greatest(sumIf(message_count, action = 'delivered'), 1) " +
                    "FROM messaging_metrics_hourly WHERE hour >= ? AND hour <= ?" + tenantClause;
            default -> null;
        };
    }
