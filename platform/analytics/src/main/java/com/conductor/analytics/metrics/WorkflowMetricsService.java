...
import com.conductor.analytics.domain.Metric;
...
/**
 * Queries ClickHouse materialized views for workflow analytics metrics.
 * All queries are tenant-scoped via mandatory tenant_id parameter.
 */
@Service
public class WorkflowMetricsService {
    ...
    public WorkflowMetricsService(@Qualifier("clickHouseDataSource") DataSource clickHouseDataSource) {
    ...
    public Map<String, Object> getWorkflowMetrics(String tenantId, Instant from, Instant to) {
        ...
        result.put("compensations", getEventCount(tenantId, "workflow", "execution", "compensated", from, to));
        return result;
    }

    private Map<String, Long> getExecutionCounts(String tenantId, Instant from, Instant to) {
        String sql = "SELECT status, sum(executions_count) as cnt FROM workflow_metrics_hourly " +
    ...
    private double getAverageLatency(String tenantId, Instant from, Instant to) {
        String sql = "SELECT sum(total_duration_ms) / sum(executions_count) as avg_ms " +
                "FROM workflow_metrics_hourly WHERE tenant_id = ? AND hour >= ? AND hour <= ? " +
    ...
    private long getEventCount(String tenantId, String domain, String entity, String action, Instant from, Instant to) {
        String sql = "SELECT count() as cnt FROM conductor_events " +
