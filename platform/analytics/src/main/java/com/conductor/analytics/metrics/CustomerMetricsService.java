...
/**
 * Queries ClickHouse materialized views for customer analytics metrics.
 * Provides growth, activity, consent status, and lifecycle insights.
 */
@Service
public class CustomerMetricsService {
    ...
    public CustomerMetricsService(@Qualifier("clickHouseDataSource") DataSource clickHouseDataSource) {
    ...
    private Map<String, Long> getActivityByAction(String tenantId, Instant from, Instant to) {
        String sql = "SELECT action, sum(customer_count) as cnt FROM customer_metrics_daily " +
    ...
    private List<Map<String, Object>> getGrowthTimeline(String tenantId, Instant from, Instant to) {
        String sql = "SELECT day, sum(customer_count) as cnt FROM customer_metrics_daily " +
