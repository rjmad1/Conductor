...
/**
 * Queries ClickHouse materialized views for messaging analytics metrics. Provides
 * sent/delivered/read/failed counts, response rate, channel performance.
 */
@Service
public class MessagingMetricsService {
  ...
  public MessagingMetricsService(
  ...
  private Map<String, Long> getMessageCountsByAction(String tenantId, Instant from, Instant to) {
    String sql =
        "SELECT action, sum(message_count) as cnt FROM messaging_metrics_hourly "
  ...
  private List<Map<String, Object>> getChannelPerformance(
      String tenantId, Instant from, Instant to) {
    String sql =
        "SELECT channel, action, sum(message_count) as cnt FROM messaging_metrics_hourly "
  ...
  private double computeRate(
      String tenantId, Instant from, Instant to, String numeratorAction, String denominatorAction) {
    String sql =
        "SELECT "
