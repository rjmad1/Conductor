...
/**
 * REST API for analytics metrics across all domains. All endpoints require authentication and are
 * tenant-scoped.
 */
@RestController
...
public class MetricsController {
  ...
  public ResponseEntity<Map<String, Object>> getWorkflowMetrics(
    ...
    return ResponseEntity.ok(
        workflowMetrics.getWorkflowMetrics(tenantId, resolveFrom(from), resolveTo(to)));
  }
  ...
  public ResponseEntity<Map<String, Object>> getMessagingMetrics(
    ...
    String tenantId = resolveTenantId();
    return ResponseEntity.ok(
        messagingMetrics.getMessagingMetrics(tenantId, resolveFrom(from), resolveTo(to)));
  }
  ...
  public ResponseEntity<Map<String, Object>> getCustomerMetrics(
    ...
    String tenantId = resolveTenantId();
    return ResponseEntity.ok(
        customerMetrics.getCustomerMetrics(tenantId, resolveFrom(from), resolveTo(to)));
  }
  ...
  public ResponseEntity<Map<String, Object>> getIntegrationMetrics(
    ...
    String tenantId = resolveTenantId();
    return ResponseEntity.ok(
        integrationMetrics.getIntegrationMetrics(tenantId, resolveFrom(from), resolveTo(to)));
  }
  ...
  public ResponseEntity<Map<String, Object>> getTenantMetrics(
    ...
    String tenantId = resolveTenantId();
    return ResponseEntity.ok(
        tenantMetrics.getTenantMetrics(tenantId, resolveFrom(from), resolveTo(to)));
  }
