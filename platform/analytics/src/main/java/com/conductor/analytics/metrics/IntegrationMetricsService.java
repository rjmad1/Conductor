...
/**
 * Queries ClickHouse materialized views for integration analytics metrics. Provides connector
 * usage, webhook health, sync success/failure rates.
 */
@Service
public class IntegrationMetricsService {
  ...
  public IntegrationMetricsService(
  ...
  private List<Map<String, Object>> getConnectorUsage(String tenantId, Instant from, Instant to) {
    String sql =
        "SELECT entity, action, sum(event_count) as cnt FROM integration_metrics_hourly "
  ...
  private Map<String, Long> getWebhookHealth(String tenantId, Instant from, Instant to) {
    String sql =
        "SELECT action, sum(event_count) as cnt FROM integration_metrics_hourly "
