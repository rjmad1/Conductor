...
/**
 * Integration tests for ConductorWorkflowImpl using Temporal's TestWorkflowEnvironment.
 * All activities are mocked to isolate workflow logic.
 */
class WorkflowExecutionIntegrationTest {
    ...
    void sequentialStepsComplete() {
        ...
        ConductorWorkflow workflow = workflowClient.newWorkflowStub(ConductorWorkflow.class,
    ...
    void workflowMarksFailedOnActivityError() {
        List<Map<String, Object>> steps = List.of(
                Map.of("name", "failing-step", "type", "INVOKE_INTEGRATION",
                        "config", Map.of("integrationId", "missing"))
        );
    ...
    void conditionFalseSkipsStep() {
        List<Map<String, Object>> steps = List.of(
                Map.of("name", "conditional-step", "type", "SEND_EVENT",
                        "config", Map.of("domain", "test"),
                        "condition", Map.of("field", "status", "operator", "EQUALS", "value", "inactive"))
        );
        ...
        verify(mockActivities, Mockito.never()).executeStep(anyString(), anyString(), anyString(), anyString(), any());
    ...
    private static class TestActivities implements ConductorActivities {
        ...
        @Override
        public Map<String, Object> executeStep(String executionId, String tenantId, String stepName,
