 ...
 * Enforces that the workflow domain does not import from other bounded contexts
 * per ARCHITECTURE_GUARDRAILS.md G-001 (No Cross-Boundary Imports).
 */
class ArchModuleBoundaryTest {
    ...
    static void loadClasses() {
        workflowClasses = new ClassFileImporter()
                .importPackages("com.conductor.workflow");
    }

    @Test
    @DisplayName("Workflow must not import from identity domain")
    void noImportFromIdentity() {
        ArchRule rule = noClasses().that().resideInAPackage("com.conductor.workflow..")
