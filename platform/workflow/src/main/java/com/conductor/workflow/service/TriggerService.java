...
/**
 * Routes incoming triggers to workflow executions. Supports event, API, webhook, schedule, timer,
 * and manual triggers.
 */
@Service
public class TriggerService {
  ...
  /**
   * Fires a trigger for the given tenant, trigger type, and input data. Finds all published
   * workflow definitions matching the trigger type and starts executions.
   */
  public void fireTrigger(UUID tenantId, TriggerType triggerType, Map<String, Object> triggerData) {
    ...
    for (WorkflowDefinition definition : definitions) {
      if (definition.getTriggerType() == triggerType) {
        try {
          ...
          triggered++;
          log.info(
              "Trigger fired: type={}, definition={}, tenant={}",
              triggerType,
              definition.getId(),
              tenantId);
        } catch (Exception e) {
          log.error(
              "Failed to start execution for trigger: definition={}, error={}",
              definition.getId(),
              e.getMessage());
          auditLogger.logEvent(
              "TRIGGER_FAILED", "workflow:" + definition.getId(), "FAILURE", e.getMessage());
        }
      }
    }
    ...
    auditLogger.logEvent(
        "TRIGGER_FIRED", "trigger:" + triggerType, "SUCCESS", "matched=" + triggered);
  }
