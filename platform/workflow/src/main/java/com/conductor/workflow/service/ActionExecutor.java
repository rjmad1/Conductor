...
/**
 * Dispatches workflow step actions by type.
 * Each action type has a dedicated handler method.
 * All actions are audited and publish metrics.
 */
@Service
public class ActionExecutor {
    ...
    public ActionExecutor(EventPublisher eventPublisher,
     ...
     */
    public Map<String, Object> execute(ActionType actionType, Map<String, Object> config, String tenantId) {
        ...
        try {
            ...
            auditLogger.logEvent("ACTION_EXECUTED", "action:" + actionType,
                    success ? "SUCCESS" : "FAILURE", "tenant=" + tenantId);
        }
    }
    ...
    private Map<String, Object> executeInvokeIntegration(Map<String, Object> config, String tenantId) {
        String integrationId = (String) config.getOrDefault("integrationId", "");
        log.info("Invoking integration {} for tenant {} (delegated to integration domain via event)",
                integrationId, tenantId);

        eventPublisher.publish("integration", "invocation", "requested", "1.0.0",
    ...
    private Map<String, Object> executeUpdateRecord(Map<String, Object> config, String tenantId) {
        ...
        log.info("Updating record: entity={}, id={}, tenant={} (delegated via event)", entity, recordId, tenantId);
    ...
    private Map<String, Object> executeGenerateNotification(Map<String, Object> config, String tenantId) {
        ...
        log.info("Generating notification: channel={}, template={}, recipient={}, tenant={}",
                channel, templateName, recipient, tenantId);

        eventPublisher.publish("messaging", "notification", "requested", "1.0.0",
