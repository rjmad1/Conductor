 ...
 * Loads the workflow definition steps from the ExecutionContext, evaluates conditions
 * via ConditionEvaluator, and dispatches each step to ConductorActivities.
 *
 * Design notes:
 * - Only uses Temporal-safe APIs (Workflow.* for time, logging, signals).
 * - No Spring beans injected — activities handle all side effects.
 * - Compensation is triggered on step failure when onFailure=COMPENSATE is set.
 */
public class ConductorWorkflowImpl implements ConductorWorkflow {
    ...
    public void execute(ExecutionContext context) {
        log.info("Workflow execution started: definitionId={}, executionId={}, tenant={}",
                context.getWorkflowDefinitionId(), context.getExecutionId(), context.getTenantId());
        ...
        List<Map<String, Object>> steps = activities.loadWorkflowSteps(
                context.getWorkflowDefinitionId().toString(),
                context.getTenantId());
        ...
        for (Map<String, Object> step : steps) {
            ...
            try {
                ...
                @SuppressWarnings("unchecked")
                Map<String, Object> onFailure = (Map<String, Object>) step.getOrDefault("onFailure", Map.of());
        ...
        if (failedStep != null) {
            if (compensate) {
                ...
                activities.markExecutionFailed(context.getExecutionId().toString(),
    ...
    private ActivityOptions buildActivityOptions() {
        return ActivityOptions.newBuilder()
                ...
                .setRetryOptions(RetryOptions.newBuilder()
                        ...
                        .setInitialInterval(Duration.ofSeconds(RetryPolicy.DEFAULT_INITIAL_INTERVAL_SECONDS))
    ...
    /**
     * Parses a condition from the step config map.
     * Returns null if no condition is specified (step always executes).
     */
    @SuppressWarnings("unchecked")
