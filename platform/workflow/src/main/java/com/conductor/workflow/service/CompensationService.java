...
/**
 * Executes compensating actions in reverse order when a workflow fails. Each completed step's
...
public class CompensationService {
  ...
  public void compensate(
      UUID executionId, Map<String, Map<String, Object>> compensationConfig, String tenantId) {
    log.info("Starting compensation for execution {} (tenant={})", executionId, tenantId);

    List<WorkflowStepExecution> completedSteps =
        stepRepository.findByExecutionIdOrderByStartedAtAsc(executionId);
    ...
    for (WorkflowStepExecution step : completedSteps) {
      ...
      try {
        ...
        Map<String, Object> compensationParams =
            (Map<String, Object>) config.getOrDefault("config", Map.of());

        actionExecutor.execute(
            ActionType.valueOf(compensationAction), compensationParams, tenantId);

        // Record compensation history
        WorkflowHistory history =
            WorkflowHistory.builder()
        ...
        log.error(
            "Compensation failed for step {} in execution {}: {}",
            step.getStepName(),
            executionId,
            e.getMessage());
        auditLogger.logEvent(
            ...
            "FAILURE",
            "step=" + step.getStepName() + " error=" + e.getMessage());
      }
    }
    ...
    log.info(
        "Compensation complete for execution {}: {} steps compensated", executionId, compensated);
  }
}
