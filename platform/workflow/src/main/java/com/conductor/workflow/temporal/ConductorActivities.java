...
/**
 * Temporal activity interface for all workflow step actions. Activities are the boundary between
 * the deterministic Temporal workflow and non-deterministic side effects (DB reads, event
 * publishing, external calls).
 */
@ActivityInterface
public interface ConductorActivities {

  /**
   * Loads workflow step definitions from the database. Invoked at workflow start — safe to retry.
   */
  @ActivityMethod
