...
/**
 * Dynamically registers Temporal workers for per-tenant task queues.
 * Task queue naming: workflow-{tenantId}
 * Workers are registered lazily on first execution for a given tenant.
 */
@Component
public class TenantWorkerRegistrar {
    ...
    public TenantWorkerRegistrar(WorkerFactory workerFactory, ConductorActivitiesImpl activitiesImpl) {
        this.workerFactory = workerFactory;
        this.activitiesImpl = activitiesImpl;
    }

    /**
     * Ensures a worker is registered for the given tenant's task queue.
     * Idempotent — safe to call repeatedly for the same tenant.
     *
