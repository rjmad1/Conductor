...
/**
 * Manages workflow execution lifecycle: start, query, cancel, replay.
 * Bridges the domain model with Temporal's durable execution runtime.
 */
@Service
public class WorkflowExecutionService {
    ...
    public WorkflowExecution startExecution(UUID definitionId, Map<String, Object> inputData) {
        ...
        auditLogger.logEvent("WORKFLOW_EXECUTION_STARTED", "execution:" + saved.getId(),
                "SUCCESS", "definition=" + definitionId);
        publishExecutionEvent(WorkflowEvents.ACTION_STARTED, saved);
        metrics.recordExecutionStarted();

        log.info("Started workflow execution {} for definition {} (temporal={})",
                saved.getId(), definitionId, workflowId);
    ...
    public List<WorkflowExecution> listExecutions(UUID definitionId, WorkflowStatus statusFilter, int limit) {
        UUID tenantId = TenantContext.getCurrentTenantId();
        ...
        return executionRepository.findByTenantIdOrderByCreatedAtDesc(tenantId, PageRequest.of(0, limit));
    }
    ...
    public WorkflowExecution cancelExecution(UUID executionId) {
        ...
        if (execution.getTemporalWorkflowId() != null) {
            try {
                WorkflowStub stub = workflowClient.newUntypedWorkflowStub(execution.getTemporalWorkflowId());
                stub.cancel();
            } catch (Exception e) {
                log.warn("Failed to cancel Temporal workflow {}: {}", execution.getTemporalWorkflowId(), e.getMessage());
            }
        }

        stateService.transition(execution, WorkflowStatus.CANCELLED, "User requested cancellation",
    ...
    public WorkflowExecution replayExecution(UUID executionId) {
        ...
        auditLogger.logEvent("WORKFLOW_EXECUTION_REPLAYED", "execution:" + executionId,
                "SUCCESS", "original=" + executionId);
    ...
    private void publishExecutionEvent(String action, WorkflowExecution execution) {
        try {
            ...
            eventPublisher.publish(WorkflowEvents.DOMAIN, WorkflowEvents.ENTITY_EXECUTION,
