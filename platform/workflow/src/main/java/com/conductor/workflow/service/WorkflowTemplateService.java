...
/**
 * Loads built-in workflow templates from classpath resources and allows tenants to instantiate them
 * as customized workflow definitions. Templates are loaded once at startup and cached in memory.
 */
@Service
public class WorkflowTemplateService {
      ...
      for (org.springframework.core.io.Resource resource : resources) {
        try {
          WorkflowTemplateDefinition template =
              objectMapper.readValue(resource.getInputStream(), WorkflowTemplateDefinition.class);
          templates.put(template.getTemplateId(), template);
          log.info(
              "Loaded workflow template: {} ({})", template.getTemplateId(), template.getName());
        } catch (Exception e) {
  ...
  /**
   * Instantiates a workflow definition from a template for the current tenant. Optionally overrides
   * template variables with provided values.
   *
   ...
   */
  public WorkflowDefinition instantiate(
      String templateId, String nameOverride, Map<String, Object> variableOverrides) {
    ...
    String name =
        nameOverride != null && !nameOverride.isBlank() ? nameOverride : template.getName();
    ...
    try {
      WorkflowDefinition definition =
          WorkflowDefinition.builder()
              .name(name)
              .description(
                  "Created from template: " + template.getName() + " v" + template.getVersion())
      ...
      throw new RuntimeException(
          "Failed to instantiate template " + templateId + ": " + e.getMessage(), e);
    }
  }
}
