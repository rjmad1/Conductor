...
/**
 * Configures the Temporal worker infrastructure.
 * Strategy: single "conductor" namespace, tenant-prefixed task queues (workflow-{tenantId}).
 * The worker registered here handles the shared/system task queue.
 * Per-tenant task queues are started dynamically via TenantWorkerRegistrar.
 */
@Configuration
public class WorkflowWorkerConfig {
    ...
    public static final String NAMESPACE = "conductor";
    ...
    /**
     * Registers the system worker on the system task queue.
     * Per-tenant workers are registered dynamically by TenantWorkerRegistrar.
     */
