...
/**
 * Manages workflow execution state transitions with validation. Enforces the state machine rules
 * and records history for auditability.
 */
@Service
public class WorkflowStateService {
  ...
  private static final Map<WorkflowStatus, Set<WorkflowStatus>> VALID_TRANSITIONS =
      Map.of(
          ...
          WorkflowStatus.WAITING,
              Set.of(WorkflowStatus.RUNNING, WorkflowStatus.CANCELLED, WorkflowStatus.FAILED),
   ...
   */
  @Transactional
  public WorkflowExecution transition(
      WorkflowExecution execution, WorkflowStatus newStatus, String reason, String actorId) {
    WorkflowStatus currentStatus = execution.getStatus();

    if (!isValidTransition(currentStatus, newStatus)) {
      throw new IllegalStateException(
          String.format(
              "Invalid state transition from %s to %s for execution %s",
    ...
    if (newStatus == WorkflowStatus.COMPLETED
        ...
        || newStatus == WorkflowStatus.COMPENSATED) {
    ...
    log.info(
        "Workflow execution {} transitioned from {} to {} (actor={})",
        execution.getId(),
        previousStatus,
        newStatus,
        actorId);
