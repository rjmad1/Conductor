...
/**
 * Manages workflow definition lifecycle: CRUD, versioning, and publishing. All mutations are
 * audited and publish domain events.
 */
@Service
public class WorkflowDefinitionService {
  ...
  public WorkflowDefinition publish(UUID id) {
    ...
    if (definition.getVersionStatus() != WorkflowVersionStatus.DRAFT) {
      throw new IllegalStateException(
          "Only DRAFT definitions can be published. Current status: "
              + definition.getVersionStatus());
    }
  ...
  public WorkflowDefinition deprecate(UUID id) {
    ...
    if (definition.getVersionStatus() != WorkflowVersionStatus.PUBLISHED) {
      throw new IllegalStateException(
          "Only PUBLISHED definitions can be deprecated. Current status: "
              + definition.getVersionStatus());
    }
  ...
  public WorkflowDefinition archive(UUID id) {
    ...
    if (definition.getVersionStatus() != WorkflowVersionStatus.DEPRECATED) {
      throw new IllegalStateException(
          "Only DEPRECATED definitions can be archived. Current status: "
              + definition.getVersionStatus());
    }
  ...
  public WorkflowDefinition clone(UUID id) {
    ...
    auditLogger.logEvent(
        "WORKFLOW_DEFINITION_CLONED",
        "workflow:" + saved.getId(),
        "SUCCESS",
        "source=" + source.getId() + " newVersion=" + saved.getVersion());

    log.info(
        "Cloned workflow definition {} -> {} (v{})",
        source.getId(),
        saved.getId(),
        saved.getVersion());
    return saved;
  ...
  public List<WorkflowDefinition> list(
      UUID tenantId, WorkflowVersionStatus statusFilter, int limit) {
  ...
  private void publishDefinitionEvent(String action, WorkflowDefinition definition) {
    try {
      ...
      eventPublisher.publish(
          ...
          WorkflowEvents.ENTITY_DEFINITION,
          action,
