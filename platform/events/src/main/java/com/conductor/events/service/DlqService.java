...
public class DlqService {
    ...
    public boolean replayRecord(UUID recordId) {
        ...
        try {
            ...
            String eventType = rootNode.get("eventType").asText(); // e.g. conductor.tenant.profile.created
            ...
            auditLogger.logEvent("DLQ_REPLAY", "DLQ_RECORD:" + recordId, "SUCCESS", "Message replayed to subject: " + subject);
            return true;
        } catch (Exception e) {
            log.error("Failed to replay DLQ record: {}", recordId, e);
            auditLogger.logEvent("DLQ_REPLAY_FAILED", "DLQ_RECORD:" + recordId, "FAILURE", e.getMessage());
            return false;
        }
    }
    ...
    public void discardRecord(UUID recordId) {
        ...
        auditLogger.logEvent("DLQ_DISCARD", "DLQ_RECORD:" + recordId, "SUCCESS", "Message marked as discarded");
    }
}
