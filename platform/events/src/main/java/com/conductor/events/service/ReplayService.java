...
public class ReplayService {
  ...
  public ReplayService(
      NatsConnectionManager connectionManager, ReplayAuditLogRepository auditLogRepository) {
  ...
  public List<String> executeReplay(
    ...
    log.info(
        "Executing event replay: tenant={}, stream={}, type={}, start={}",
        tenantId,
        streamName,
        replayType,
        startValue);
    ...
    try {
      ...
      ConsumerConfiguration.Builder ccb =
          ConsumerConfiguration.builder().filterSubject(filterSubject);

      if ("SEQUENCE".equalsIgnoreCase(replayType)) {
        long seq = Long.parseLong(startValue);
        ccb.deliverPolicy(DeliverPolicy.ByStartSequence).startSequence(seq);
        ...
        ccb.deliverPolicy(DeliverPolicy.ByStartTime).startTime(startZoned);
      } else {
        ccb.deliverPolicy(DeliverPolicy.All);
      }

      PullSubscribeOptions pso =
          PullSubscribeOptions.builder().configuration(ccb.build()).stream(streamName).build();
