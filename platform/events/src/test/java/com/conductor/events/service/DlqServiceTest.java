...
class DlqServiceTest {
    ...
    void testReplayRecordSuccess() throws Exception {
        ...
        record.setStatus("PENDING");
        record.setPayload("{\"eventType\":\"conductor.tenant.profile.created\",\"tenantId\":\"" + tenantId + "\",\"payload\":\"{}\"}");
        ...
        verify(conn).publish(eq("conductor." + tenantId + ".tenant.profile.created"), any(byte[].class));
        verify(auditLogger).logEvent(eq("DLQ_REPLAY"), eq("DLQ_RECORD:" + recordId), eq("SUCCESS"), anyString());
    }
    ...
    void testDiscardRecord() {
        ...
        verify(repository).save(record);
        verify(auditLogger).logEvent(eq("DLQ_DISCARD"), eq("DLQ_RECORD:" + recordId), eq("SUCCESS"), anyString());
    }
}
