...
/**
 * Shared probe logic for all connector health checks.
 * Performs a lightweight GET against a provider meta-endpoint and
 * maps the HTTP response to a ConnectorStatus:
 *   2xx              → HEALTHY
 *   4xx (incl. 401)  → HEALTHY  (API is up; auth context is tenant-level)
 *   429              → DEGRADED  (provider rate-limiting)
 *   5xx              → UNAVAILABLE
 *   IOException/timeout → UNAVAILABLE, retried once before giving up
 */
public final class ConnectorHealthProbe {
    ...
    public static ConnectorHealthResult probe(String connectorType, String url,
        ...
        try {
            ...
            return new ConnectorHealthResult(status,
                    status == ConnectorStatus.HEALTHY ? "OK" : status.name(), latency);
        } catch (ResourceAccessException firstEx) {
            log.debug("{} health probe transient failure, retrying: {}", connectorType, firstEx.getMessage());
            try {
                ConnectorStatus status = attempt(url, restTemplate);
                long latency = System.currentTimeMillis() - start;
                log.debug("{} health probe retry → {} ({}ms)", connectorType, status, latency);
                return new ConnectorHealthResult(status,
                        status == ConnectorStatus.HEALTHY ? "OK (retry)" : status.name(), latency);
            } catch (Exception retryEx) {
                long latency = System.currentTimeMillis() - start;
                log.warn("{} health probe failed after retry: {}", connectorType, retryEx.getMessage());
                return ConnectorHealthResult.unavailable("Unreachable after retry: " + retryEx.getMessage(), latency);
            }
        } catch (Exception ex) {
            long latency = System.currentTimeMillis() - start;
