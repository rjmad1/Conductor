...
public class IntegrationController {
  ...
  public ResponseEntity<Credential> saveCredential(
  ...
  public ResponseEntity<Map<String, String>> authorizeOAuth(
      @RequestParam UUID integrationId, @RequestParam String authUrl) {
  ...
  public ResponseEntity<String> oauthCallback(
      ...
      @RequestParam(
              value = "redirect_uri",
              required = false,
              defaultValue = "/api/v1/integrations/oauth/callback")
          String redirectUri) {
    ...
    try {
      ...
      eventPublisher.publish(
          "integration", "integration", "connected", "v1", Map.of("integrationId", integrationId));
  ...
  public ResponseEntity<WebhookSubscription> subscribeWebhook(
    ...
    connectorRegistry
        .getAdapter(integration.getConnector().getType())
  ...
  public ResponseEntity<List<ExecutionHistory>> getHistory(@PathVariable UUID id) {
    ...
    return ResponseEntity.ok(
        executionHistoryRepository.findByIntegrationIdAndTenantId(id, tenantId));
  }
