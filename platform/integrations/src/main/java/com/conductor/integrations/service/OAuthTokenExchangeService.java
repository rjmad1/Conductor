...
/**
 * Exchanges an OAuth authorization code for real access/refresh tokens by calling
 * the provider's token endpoint. Replaces the prior placeholder that stored fake
 * mock tokens in the production callback path.
 *
 * Requires the integration's OAuthConnection to have tokenUrl, clientId, and
 * clientSecret pre-configured (stored via /credentials before OAuth initiation).
 */
@Service
public class OAuthTokenExchangeService {
    ...
    /**
     * Exchanges the authorization code for tokens and persists them (encrypted) on the
     * existing OAuthConnection record. The record must already exist with tokenUrl,
     * clientId, and clientSecret configured; if not, an IllegalStateException is thrown.
     *
     ...
     */
    public OAuthConnection exchange(UUID integrationId, UUID tenantId, String code, String redirectUri) {
        OAuthConnection conn = oauthConnectionRepository
                .findByIntegrationIdAndTenantId(integrationId, tenantId)
                .orElseThrow(() -> new IllegalStateException(
                        "No OAuth connection record found for integration " + integrationId
        ...
        if (tokenUrl == null || tokenUrl.isBlank()) {
            throw new IllegalStateException(
                    "tokenUrl is not configured on integration " + integrationId
                    + ". Store OAuth config via POST /credentials before initiating authorization.");
        }
        if (clientId == null || clientId.isBlank() || encryptedSecret == null || encryptedSecret.isBlank()) {
            throw new IllegalStateException(
        ...
        try {
            ...
            auditLogger.logEvent("OAUTH_TOKEN_EXCHANGE_FAILED", "integration:" + integrationId, "FAILURE",
        ...
        String accessToken = Optional.ofNullable(tokenResponse.get("access_token"))
                .map(Object::toString)
                .orElseThrow(() -> new IllegalStateException(
                        "OAuth provider response missing access_token for integration " + integrationId));
        String refreshToken = Optional.ofNullable(tokenResponse.get("refresh_token"))
                .map(Object::toString)
                .orElse(null);
        long expiresIn = Optional.ofNullable(tokenResponse.get("expires_in"))
                .map(v -> Long.parseLong(v.toString()))
                .orElse(3600L);
        String scope = Optional.ofNullable(tokenResponse.get("scope"))
                .map(Object::toString)
                .orElse(null);
        ...
        auditLogger.logEvent("OAUTH_TOKEN_EXCHANGE_SUCCESS", "integration:" + integrationId, "SUCCESS",
