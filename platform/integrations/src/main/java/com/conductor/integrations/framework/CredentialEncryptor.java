...
public class CredentialEncryptor {
  ...
  public CredentialEncryptor(
      @Value("${INTEGRATION_ENCRYPTION_KEY:defaultValueDefaultValueDefaultVa}") String keyString) {
    byte[] keyBytes = keyString.getBytes(StandardCharsets.UTF_8);
    if (keyBytes.length != 32) {
      log.warn(
          "INTEGRATION_ENCRYPTION_KEY must be exactly 32 bytes long for AES-256 GCM. Resizing to 32 bytes.");
