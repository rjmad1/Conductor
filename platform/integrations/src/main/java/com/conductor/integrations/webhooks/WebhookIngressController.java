...
public class WebhookIngressController {
    ...
    public ResponseEntity<Void> handleIngress(
        ...
        try {
            ...
            Optional<WebhookSubscription> subOpt = subscriptionRepository
                    .findByIntegrationConnectorTypeAndEventNameAndTenantId(connectorType.toLowerCase(), eventName, tenantId);

            if (subOpt.isEmpty()) {
                log.warn("No active subscription found for connector: {}, event: {}, tenant: {}", connectorType, eventName, tenantId);
            ...
            if (!isValid) {
                ...
                auditLogger.logEvent("WEBHOOK_REJECTED", "webhook:" + connectorType, "FAILURE", "Signature invalid");
            ...
            boolean published = eventPublisher.publish("integration", connectorType.toLowerCase(), action, "v1", payloadMap);

            if (published) {
                auditLogger.logEvent("WEBHOOK_RECEIVED", "webhook:" + connectorType, "SUCCESS", "Webhook event processed: " + eventName);
    ...
    private String getActionName(String connector, String eventName) {
        ...
        if ("razorpay".equalsIgnoreCase(connector)) {
            ...
            if ("payment.captured".equalsIgnoreCase(eventName) || "payment.completed".equalsIgnoreCase(eventName)) return "payment_completed";
            ...
            if ("refund.processed".equalsIgnoreCase(eventName) || "refund.completed".equalsIgnoreCase(eventName)) return "refund_completed";
        }
