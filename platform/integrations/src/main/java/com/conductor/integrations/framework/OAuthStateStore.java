 ...
 *
 * When an OAuth authorization is initiated, a cryptographically-random state token is
 * generated and stored here along with the integrationId and tenantId it belongs to.
 * The callback endpoint validates the state token exists in this store and consumes
 * it (one-time use). Tokens that are never redeemed expire after {@code oauth.state.ttl-seconds}.
 *
 * LIMITATION: this is process-local. In a horizontally-scaled deployment, sticky sessions
 * or a shared store (e.g. Redis) should be used instead. Flag: oauth.state.distributed=true.
 */
@Component
public class OAuthStateStore {
    ...
    /**
     * Generates a random state token, stores it, and returns the token for inclusion
     * in the authorization redirect URL.
     */
    ...
    /**
     * Validates the token and, if valid and unexpired, removes it and returns the
     * associated state. Returns empty if the token is unknown, expired, or already used.
     */
