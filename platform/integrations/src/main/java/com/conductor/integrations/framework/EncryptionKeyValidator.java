...
/**
 * Fails application startup when the encryption key is the well-known insecure default and the
 * active Spring profile is not dev/test/local.
 *
 * <p>Set INTEGRATION_ENCRYPTION_KEY to a cryptographically-random 32-byte value before deploying to
...
public class EncryptionKeyValidator implements InitializingBean {
  ...
  public EncryptionKeyValidator(
      @Value("${INTEGRATION_ENCRYPTION_KEY:defaultValueDefaultValueDefaultVa}")
  ...
  public void afterPropertiesSet() {
    ...
    if (!isSafeProfile) {
      String msg =
          "FATAL: INTEGRATION_ENCRYPTION_KEY is the insecure default. "
    ...
    log.warn(
        "INTEGRATION_ENCRYPTION_KEY is using the insecure default. "
