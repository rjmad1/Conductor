...
class IntegrationExceptionHandlerTest {
    ...
    void unknownIntegration_returns404WithProblemDetail() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/integrations/" + id + "/health")
                        .with(csrf())
    ...
    void executeWithUnknownAction_returns400WithProblemDetail() throws Exception {
        UUID id = UUID.randomUUID();
        com.conductor.integrations.domain.Connector connector = new com.conductor.integrations.domain.Connector();
        connector.setType("shopify");
        com.conductor.integrations.domain.Integration integration = new com.conductor.integrations.domain.Integration();
        integration.setConnector(connector);

        when(integrationRepository.findByIdAndTenantId(eq(id), any()))
                .thenReturn(Optional.of(integration));

        org.mockito.stubbing.Answer<?> throwUnsupported = inv ->
        ...
        when(adapter.execute(any(), eq("bad-action"), any()))
                .thenAnswer(throwUnsupported);
        when(connectorRegistry.getAdapter("shopify")).thenReturn(Optional.of(adapter));

        mockMvc.perform(post("/api/v1/integrations/" + id + "/execute")
