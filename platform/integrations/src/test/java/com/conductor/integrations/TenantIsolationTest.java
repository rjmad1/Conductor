...
class TenantIsolationTest {
  ...
  @MockBean private ConnectorRepository connectorRepository;

  @MockBean private IntegrationRepository integrationRepository;

  @MockBean private ConnectionRepository connectionRepository;

  @MockBean private WebhookSubscriptionRepository webhookSubscriptionRepository;

  @MockBean private ExecutionHistoryRepository executionHistoryRepository;

  @MockBean private CredentialService credentialService;

  @MockBean private ConnectorRegistry connectorRegistry;
  ...
  @MockBean private com.conductor.integrations.framework.OAuthStateStore oauthStateStore;
  ...
  void givenActiveTenant_whenGetHistory_thenOk() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/integrations/" + integrationId + "/history")
  ...
  void givenActiveTenant_whenGetCrossTenantHealth_thenReturn404() throws Exception {
    ...
    when(integrationRepository.findByIdAndTenantId(crossTenantIntegrationId, tenantId))
        .thenReturn(Optional.empty());

    mockMvc
        .perform(
            get("/api/v1/integrations/" + crossTenantIntegrationId + "/health")
                .header("X-Tenant-ID", tenantId.toString())
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON))
  ...
  void givenActiveTenant_whenExecuteCrossTenant_thenReturn404() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/integrations/" + crossTenantIntegrationId + "/execute")
                .param("action", "test")
                .header("X-Tenant-ID", tenantId.toString())
                .with(csrf())
                .content("{}")
