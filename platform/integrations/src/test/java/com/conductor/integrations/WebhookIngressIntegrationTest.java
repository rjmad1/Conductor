...
class WebhookIngressIntegrationTest {
    ...
    @MockBean
    private WebhookReplayProtector replayProtector;
    ...
    void givenWebhookIngress_whenSignatureAndReplayValid_thenAcceptAndPublish() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/integrations/webhooks/ingress/shopify/" + tenantId)
        ...
        verify(eventPublisher).publish(eq("integration"), eq("shopify"), eq("order_created"), eq("v1"), any());
    }
}
