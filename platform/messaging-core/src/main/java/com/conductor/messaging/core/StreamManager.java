...
public class StreamManager implements InitializingBean {
    ...
    public void initializeStreams() {
        ...
        try {
            ...
            List<StreamDefinition> streamDefinitions = Arrays.asList(
                    new StreamDefinition("TENANT_STREAM", Arrays.asList("conductor.*.tenant.>"), Duration.ofDays(30)),
                    new StreamDefinition("CUSTOMER_STREAM", Arrays.asList("conductor.*.customer.>"), Duration.ofDays(30)),
                    new StreamDefinition("MESSAGING_STREAM", Arrays.asList("conductor.*.messaging.>", "conductor.*.message.>"), Duration.ofDays(7)),
                    new StreamDefinition("WORKFLOW_STREAM", Arrays.asList("conductor.*.workflow.>"), Duration.ofDays(14)),
                    new StreamDefinition("INTEGRATION_STREAM", Arrays.asList("conductor.*.integration.>"), Duration.ofDays(7)),
                    new StreamDefinition("ANALYTICS_STREAM", Arrays.asList("conductor.*.analytics.>"), Duration.ofDays(7)),
                    new StreamDefinition("AUDIT_STREAM", Arrays.asList("conductor.*.audit.>"), Duration.ofDays(90)),
                    new StreamDefinition("AI_STREAM", Arrays.asList("conductor.*.ai.>"), Duration.ofDays(7)),
