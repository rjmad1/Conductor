...
class SegmentServiceTest {
    ...
    void setUp() {
        segmentService = new SegmentService(
                segmentRepository, customerSegmentRepository, customerRepository,
                customerTagRepository, customerService, timelineService, eventPublisher, auditLogger
        );
    }
    ...
    void testCreateSegment() {
        ...
        Segment segment = segmentService.createSegment("VIP Customers", SegmentType.STATIC, null, "desc");
    ...
    void testCreateSegment_SlugExists_ThrowsException() {
        ...
        assertThrows(IllegalArgumentException.class, () -> segmentService.createSegment("VIP Customers", SegmentType.STATIC, null, "desc"));
    }
    ...
    void testAddCustomerToSegment() {
        ...
        when(customerSegmentRepository.existsByCustomerIdAndSegmentId(customerId, segmentId)).thenReturn(false);
    ...
    void testRemoveCustomerFromSegment() {
        ...
        when(customerService.requireCustomer(customerId)).thenReturn(customer);
        when(customerSegmentRepository.existsByCustomerIdAndSegmentId(customerId, segmentId)).thenReturn(true);
        when(segmentRepository.findById(segmentId)).thenReturn(Optional.of(segment));
    ...
    void testRecomputeTagBasedSegment() {
        ...
        when(segmentRepository.findById(segmentId)).thenReturn(Optional.of(segment));
        when(customerTagRepository.findCustomerIdsByTagId(tagId)).thenReturn(List.of(customer1, customer2));
        when(customerSegmentRepository.findCustomerIdsBySegmentId(segmentId)).thenReturn(List.of(customer1)); // customer1 already in segment, customer2 is not.
        ...
        verify(customerSegmentRepository, never()).deleteByCustomerIdAndSegmentId(any(UUID.class), any(UUID.class));
    }
}
