...
class CustomerTimelineServiceTest {
    ...
    @Mock
    private CustomerTimelineRepository timelineRepository;
    ...
    void testRecord() {
        ...
        when(timelineRepository.save(any(CustomerTimeline.class))).thenAnswer(invocation -> {
            CustomerTimeline t = invocation.getArgument(0);
            t.setId(UUID.randomUUID());
            return t;
        });

        CustomerTimeline entry = timelineService.record(
    ...
    void testGetTimelineByEventType() {
        ...
        when(timelineRepository.findByCustomerIdAndEventTypeOrderByOccurredAtDesc(customerId, TimelineEventType.CONSENT_GRANTED))
                .thenReturn(List.of(t1));

        List<CustomerTimeline> list = timelineService.getTimelineByEventType(customerId, TimelineEventType.CONSENT_GRANTED);
    ...
    void testGetTimelineInRange() {
        ...
        when(timelineRepository.findByCustomerIdAndOccurredAtBetweenOrderByOccurredAtDesc(customerId, from, to))
                .thenReturn(List.of(t1));
