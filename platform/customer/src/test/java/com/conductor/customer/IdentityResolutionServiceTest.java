...
class IdentityResolutionServiceTest {
  ...
  void setUp() {
    identityResolutionService =
        new IdentityResolutionService(contactRepository, identifierRepository);
  }

  @Test
  void testNormalizeEmail() {
    assertEquals(
        "test@example.com", identityResolutionService.normalizeEmail("  TEST@example.com  "));
  ...
  void testResolveByExternalId() {
    ...
    when(identifierRepository.findByIdentifierTypeAndIdentifierHash(identifierType, hash))
        .thenReturn(Optional.of(identifier));

    Optional<UUID> resolved =
        identityResolutionService.resolveByExternalId(externalId, sourceSystem);
    assertTrue(resolved.isPresent());
    assertEquals(customerId, resolved.get());
  }
}
