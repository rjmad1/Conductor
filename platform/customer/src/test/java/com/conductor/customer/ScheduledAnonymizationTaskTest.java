...
class ScheduledAnonymizationTaskTest {
    ...
    @BeforeEach
    void setUp() {
        task = new ScheduledAnonymizationTask(customerRepository, contactRepository, identifierRepository);
        TenantContext.clear();
    }
    ...
    void testAnonymizationProcess_WithDeletedCustomers_Succeeds() {
        ...
        when(customerRepository.findByStatus(CustomerStatus.DELETED)).thenReturn(Collections.singletonList(customer));
        when(contactRepository.findByCustomerId(customerId)).thenReturn(Collections.singletonList(contact));
        when(identifierRepository.findByCustomerId(customerId)).thenReturn(Collections.singletonList(identifier));
