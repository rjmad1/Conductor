 ...
 *
 * <p>PII fields (firstName, lastName) are encrypted at rest via PiiEncryptedConverter (SG-006).
 * displayName is a non-PII computed label for search/display (may contain first name only or
 * alias).
 *
 * <p>Extends TenantAwareEntity → tenant_id injected from TenantContext on @PrePersist.
...
public class Customer extends TenantAwareEntity {
  ...
  /**
   * Optional identifier from source system (CRM, Shopify, etc.). Unique per tenant + source_system
   * combination.
   */
  ...
  /**
   * Non-sensitive display label for search results and UI. Typically "First L." or a business
   * alias. Not encrypted.
   */
  ...
  /**
   * When status = MERGED, this references the surviving customer record. The merged-from customer
   * is soft-deleted (status = MERGED).
   */
