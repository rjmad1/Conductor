...
public class TagService {
    ...
    public void assignTag(UUID customerId, UUID tagId, String assignedBy) {
        ...
        timelineService.record(customerId, TimelineEventType.TAG_ASSIGNED,
                "customer-service", "Tag assigned: " + tagId,
                String.format("{\"tagId\":\"%s\"}", tagId));

        eventPublisher.publishEvent(CustomerEvents.DOMAIN, CustomerEvents.ENTITY_TAG,
                CustomerEvents.ACTION_ASSIGNED,
                String.format("{\"customerId\":\"%s\",\"tagId\":\"%s\"}", customerId, tagId));

        auditLogger.logEvent("ASSIGN_TAG", "CUSTOMER:" + customerId + ":TAG:" + tagId,
    ...
    public void removeTag(UUID customerId, UUID tagId) {
        ...
        timelineService.record(customerId, TimelineEventType.TAG_REMOVED,
                "customer-service", "Tag removed: " + tagId,
                String.format("{\"tagId\":\"%s\"}", tagId));
        ...
        auditLogger.logEvent("REMOVE_TAG", "CUSTOMER:" + customerId + ":TAG:" + tagId,
                "SUCCESS", "Tag removed");
    }
