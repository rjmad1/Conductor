 ...
 *
 * <p>Search strategies: 1. Phone search → identity resolution via value_hash (O(1), exact) 2. Email
 * search → identity resolution via value_hash (O(1), exact) 3. Name search → PostgreSQL full-text
 * search via tsvector (approximate) 4. Tag search → join via customer_tags 5. Segment search→ join
 * via customer_segments
 *
 * <p>All queries are tenant-scoped by the Hibernate @Filter (TenantAwareEntity). No cross-tenant
 * results are ever returned.
 *
 * <p>Results are de-duplicated using LinkedHashSet to preserve ranking order when multiple search
...
public class CustomerSearchService {
  ...
  public List<Customer> searchByPhone(String phone) {
    String hash =
        identityResolutionService.hashPhone(identityResolutionService.normalizePhone(phone));
  ...
  public List<Customer> searchByEmail(String email) {
    String hash =
        identityResolutionService.hashEmail(identityResolutionService.normalizeEmail(email));
    return contactRepository
  ...
  /**
   * Multi-field search: runs applicable strategies based on provided params. Results are merged and
   * de-duplicated. Order: phone > email > name > tag > segment.
   */
  public List<Customer> search(
      String phone, String email, String name, UUID tagId, UUID segmentId) {
