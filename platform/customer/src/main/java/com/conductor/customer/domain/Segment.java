 ...
 *
 * rules (JSONB) is the rule DSL evaluated by SegmentService for DYNAMIC/RULE_BASED segments.
 * Structure: {"conditions": [{"field":"status","operator":"EQ","value":"ACTIVE"}], "logic":"AND"}
 *
 * customerCount is a cached/denormalised count refreshed by SegmentService.
 * TAG_BASED segments derive membership from tag assignment (no rules JSONB needed).
 */
@Entity
