 ...
 *
 * Extends the Customer model without schema changes.
 * The value field is JSONB to support numbers, booleans, arrays, and nested objects.
 * dataType is a hint for UI rendering and validation (STRING, NUMBER, BOOLEAN, DATE, JSON).
 */
@Entity
@Table(name = "customer_attributes", indexes = {
    @Index(name = "idx_attributes_customer_key", columnList = "tenant_id, customer_id, attribute_key", unique = true),
