 ...
 *   1. Consent records are APPEND-ONLY. No update or delete is ever called.
 *   2. Current consent status is derived from the most recent record.
 *   3. Every consent change is audited and publishes a NATS event.
 *   4. Consent is versioned: each record carries a consentVersion string.
 *   5. Revoking a consent that was never granted throws ConsentException.
 *
 * Compliance notes:
 *   - DPDP §12 Right to Erasure: customer deletion triggers consent record anonymisation
 *     (handled by CustomerService, not here — consent history must be preserved for compliance).
 *   - Meta WhatsApp Policy: opt-out keywords must be processed within 5 seconds.
 *     The Messaging domain publishes opt-out events; this service reacts via event consumer.
 */
@Service
public class ConsentService {
    ...
    @Transactional
    public ConsentRecord grantConsent(UUID customerId, ConsentType consentType, String channel,
                                      String legalBasis, String consentVersion,
                                      String ipAddress, String userAgent, String metadataJson) {
        customerService.requireCustomer(customerId);

        ConsentRecord record = buildRecord(customerId, consentType, ConsentAction.GRANTED,
                channel, legalBasis, consentVersion, ipAddress, userAgent, metadataJson);
        ConsentRecord saved = consentRepository.save(record);

        timelineService.record(customerId, TimelineEventType.CONSENT_GRANTED,
                "customer-service",
                "Consent granted: " + consentType.name(),
                String.format("{\"type\":\"%s\",\"version\":\"%s\"}", consentType, consentVersion));

        eventPublisher.publishEvent(CustomerEvents.DOMAIN, CustomerEvents.ENTITY_CONSENT,
                CustomerEvents.ACTION_GRANTED,
                String.format("{\"customerId\":\"%s\",\"consentType\":\"%s\",\"version\":\"%s\"}",
                        customerId, consentType, consentVersion));

        auditLogger.logEvent("CONSENT_GRANTED", "CUSTOMER:" + customerId + ":CONSENT:" + consentType,
                "SUCCESS", "Consent granted version=" + consentVersion);
    ...
    @Transactional
    public ConsentRecord revokeConsent(UUID customerId, ConsentType consentType, String channel,
                                       String consentVersion, String ipAddress, String userAgent,
                                       String metadataJson) {
        customerService.requireCustomer(customerId);

        // Validate: can only revoke if currently granted
        Optional<ConsentRecord> latest = consentRepository
        ...
        if (latest.isEmpty() || latest.get().getAction() != ConsentAction.GRANTED) {
            throw new ConsentException(
                "Cannot revoke consent for " + consentType + " — no active grant exists for customer " + customerId);
        }

        ConsentRecord record = buildRecord(customerId, consentType, ConsentAction.REVOKED,
                channel, null, consentVersion, ipAddress, userAgent, metadataJson);
        ConsentRecord saved = consentRepository.save(record);

        timelineService.record(customerId, TimelineEventType.CONSENT_REVOKED,
                "customer-service",
        ...
        auditLogger.logEvent("CONSENT_REVOKED", "CUSTOMER:" + customerId + ":CONSENT:" + consentType,
    ...
    public List<ConsentRecord> getConsentHistoryByType(UUID customerId, ConsentType consentType) {
        return consentRepository.findByCustomerIdAndConsentTypeOrderByCreatedAtDesc(customerId, consentType);
    }

    // ── Builder ───────────────────────────────────────────────────────────────

    private ConsentRecord buildRecord(UUID customerId, ConsentType consentType, ConsentAction action,
                                      String channel, String legalBasis, String consentVersion,
                                      String ipAddress, String userAgent, String metadataJson) {
