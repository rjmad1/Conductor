 ...
 *
 * <p>Each record represents a single consent event (GRANTED or REVOKED). Records are NEVER updated
 * or deleted. Immutability is enforced at two levels: 1. Service layer: ConsentService only ever
 * calls repository.save(newRecord) 2. Database layer: PostgreSQL trigger blocks UPDATE/DELETE on
 * consent_records table
 *
 * <p>The "current" consent status is derived by reading the latest record per (tenant_id,
 * customer_id, consent_type) ordered by created_at DESC.
 *
 * <p>consentVersion allows versioned policy updates (e.g. "privacy-policy-v2").
 */
@Entity
@Table(
    ...
    indexes = {
      @Index(
          name = "idx_consent_customer_type",
          columnList = "tenant_id, customer_id, consent_type"),
      @Index(
          name = "idx_consent_customer_created",
          columnList = "tenant_id, customer_id, created_at")
    })
@Getter
@Setter
