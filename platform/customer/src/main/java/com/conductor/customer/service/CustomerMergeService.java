 ...
 *
 * <p>Merge is a significant and auditable operation. It: 1. Validates both customers exist and are
 * in mergeable states 2. Migrates all contacts, tags, segments, consents, attributes, preferences
 * from source → target (duplicates skipped) 3. Records a timeline entry on both source and target
 * customers 4. Marks source customer as MERGED with mergedIntoId pointing to target 5. Publishes
 * customer.profile.merged event 6. Logs audit record
 *
 * <p>Recovery: the merge can be audited via the timeline. The source customer record is
 * soft-deleted (MERGED status), not physically removed.
 *
 * <p>Cross-tenant merges are impossible — TenantAwareEntity Hibernate filter scopes all repository
...
public class CustomerMergeService {
  ...
  public Customer merge(UUID sourceId, UUID targetId) {
    ...
    eventPublisher.publishEvent(
        ...
        String.format(
            "{\"sourceId\":\"%s\",\"targetId\":\"%s\",\"mergedAt\":\"%s\"}",
            sourceId, targetId, Instant.now()));

    auditLogger.logEvent(
        "MERGE", "CUSTOMER:" + sourceId + "->CUSTOMER:" + targetId, "SUCCESS", "Customer merged");
  ...
  private void migrateContacts(UUID sourceId, UUID targetId) {
    ...
    sourceContacts.forEach(
        sc -> {
          boolean duplicate =
              targetContacts.stream()
                  .anyMatch(
                      tc ->
                          tc.getType() == sc.getType()
                              && tc.getValueHash().equals(sc.getValueHash()));
          if (!duplicate) {
  ...
  private void migrateAttributes(UUID sourceId, UUID targetId) {
    ...
    sourceAttrs.forEach(
        sa -> {
          ...
          attributeRepository
              .findByCustomerIdAndKey(targetId, sa.getKey())
              .ifPresentOrElse(
                  existing -> {
