...
/**
 * CustomerTimelineService — append-only customer activity log. This service is called by all other
 * customer domain services to record events. External services (Messaging, Workflow) should publish
 * events that trigger timeline entries via the event consumer.
 */
@Service
public class CustomerTimelineService {
  ...
  @Transactional
  public CustomerTimeline record(
      UUID customerId,
  ...
  public List<CustomerTimeline> getTimelineByEventType(
      UUID customerId, TimelineEventType eventType) {
    return timelineRepository.findByCustomerIdAndEventTypeOrderByOccurredAtDesc(
        customerId, eventType);
  }

  public List<CustomerTimeline> getTimelineInRange(UUID customerId, Instant from, Instant to) {
    return timelineRepository.findByCustomerIdAndOccurredAtBetweenOrderByOccurredAtDesc(
        customerId, from, to);
  }
}
