 ...
 *
 * All lookups are performed on SHA-256 hashes of normalized values.
 * Raw PII is never exposed during resolution — identity check is purely hash-based.
 *
 * Normalization rules:
 *   - Email: trim + lowercase
 *   - Phone: strip non-digit chars, ensure leading + for E.164
 *   - External ID: trim
 *
 * All resolution operations are tenant-scoped via Hibernate @Filter (TenantAwareEntity).
 * Audit trail is maintained by callers (CustomerService, CustomerMergeService).
 */
@Service
public class IdentityResolutionService {
    ...
    public Optional<UUID> resolveByExternalId(String externalId, String sourceSystem) {
        String identifierType = "EXTERNAL_ID:" + (sourceSystem != null ? sourceSystem.toUpperCase() : "UNKNOWN");
        String hash = hashNormalized(externalId.trim());
        return identifierRepository.findByIdentifierTypeAndIdentifierHash(identifierType, hash)
                .map(CustomerIdentifier::getCustomerId);
    }

    /**
     * Register an email identifier for a customer.
     * Creates an entry in customer_contacts (with hash for lookup) and
     * customer_identifiers (for the identity index).
     * Called by ContactService when a new email contact is added.
     */
