 ...
 *
 * Multiple contacts per customer are supported.
 * Only one contact per type may be marked isPrimary = true.
 *
 * contact_value is encrypted at rest (Tier-1 PII).
 * value_hash is a SHA-256 hash of the normalized contact value, used for identity resolution
 * lookups without decrypting the stored PII.
 */
@Entity
@Table(name = "customer_contacts", indexes = {
    @Index(name = "idx_contacts_customer_id",    columnList = "tenant_id, customer_id"),
    @Index(name = "idx_contacts_type_primary",   columnList = "tenant_id, contact_type, is_primary"),
...
public class CustomerContact extends TenantAwareEntity {
    ...
    /**
     * SHA-256 hash of the normalized contact value.
     * Used for O(1) identity resolution without touching the encrypted PII.
     * Stored in plain text — hash is not reversible.
     */
