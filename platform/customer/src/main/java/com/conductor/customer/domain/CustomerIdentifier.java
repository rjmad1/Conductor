 ...
 *
 * Stores hashed versions of external identifiers (email, phone, external CRM ID)
 * for fast O(1) duplicate detection and merge candidate lookup.
 *
 * The identifier_value is the SHA-256 hash of the normalized raw value.
 * Raw values are never stored here — they live encrypted in CustomerContact.
 *
 * Source system mapping enables tracking which external system contributed the identifier.
 */
@Entity
@Table(name = "customer_identifiers", indexes = {
    @Index(
...
public class CustomerIdentifier extends TenantAwareEntity {
    ...
    /**
     * SHA-256 hash of normalized identifier value.
     * For email: lower-cased. For phone: E.164 normalised.
     */
