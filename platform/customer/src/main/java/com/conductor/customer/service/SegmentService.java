 ...
 *
 * <p>Segment types: - STATIC: manual membership managed via API - DYNAMIC / RULE_BASED: membership
 * computed from customer attributes (uses ConditionEvaluator from :shared:rules — scheduled or
 * event-triggered recompute) - TAG_BASED: membership derived from tag assignments - BEHAVIOR_BASED:
 * membership based on timeline event history (future phase)
 *
 * <p>Dynamic segment recompute is intentionally deferred (not real-time) to avoid write
 * amplification on every customer update. Recompute is triggered by: 1. Scheduled job
 * (future: @Scheduled annotation) 2. Explicit API call: POST /api/v1/segments/{id}/recompute
 *
...
public class SegmentService {
  ...
  public Segment createSegment(
      String name, SegmentType type, String rulesJson, String description) {
    ...
    auditLogger.logEvent(
        "CREATE_SEGMENT", "SEGMENT:" + saved.getId(), "SUCCESS", "Segment created: " + slug);
    return saved;
  }
  ...
  public void deleteSegment(UUID segmentId) {
    Segment segment = requireSegment(segmentId);
    customerSegmentRepository.findBySegmentId(segmentId).forEach(customerSegmentRepository::delete);
  ...
  public void removeCustomerFromSegment(UUID customerId, UUID segmentId) {
    ...
    if (!customerSegmentRepository.existsByCustomerIdAndSegmentId(customerId, segmentId)) {
      throw new CustomerNotFoundException(
          "Customer " + customerId + " not in segment " + segmentId);
    }
    ...
    timelineService.record(
        customerId,
        TimelineEventType.SEGMENT_REMOVED,
        "customer-service",
        "Removed from segment: " + segmentId,
        String.format("{\"segmentId\":\"%s\"}", segmentId));
  ...
  /**
   * Recompute TAG_BASED segment membership. For the given tag-based segment, the rules JSON
   * contains {"tagId":"<uuid>"}. All customers with that tag are added; customers without are
   * removed.
   */
  @Transactional
