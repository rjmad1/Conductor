...
public interface ConsentRecordRepository extends JpaRepository<ConsentRecord, UUID> {
    ...
    List<ConsentRecord> findByCustomerIdAndConsentTypeOrderByCreatedAtDesc(UUID customerId, ConsentType consentType);

    /**
     * Derives current consent status: returns the most recent record for the
     * given customer and consent type (action = GRANTED means currently opted in).
     */
    @Query("""
