...
public class ContactController {
    ...
    public ResponseEntity<List<ContactResponse>> getContacts(@PathVariable UUID customerId) {
        List<ContactResponse> contacts = contactService.getContacts(customerId)
                .stream()
                .map(this::toResponse)
    ...
    @PostMapping
    public ResponseEntity<ContactResponse> addContact(@PathVariable UUID customerId,
                                                      @Valid @RequestBody AddContactRequest request) {
        CustomerContact contact = contactService.addContact(
                customerId,
                request.type(),
                request.value(),
                request.label(),
    ...
    public ResponseEntity<ContactResponse> updateContact(@PathVariable UUID customerId,
                                                         @PathVariable UUID contactId,
                                                         @Valid @RequestBody UpdateContactRequest request) {
        CustomerContact contact = contactService.updateContact(
                customerId,
                contactId,
                request.label(),
                request.isPrimary()
    ...
    public ResponseEntity<Void> setPrimaryContact(@PathVariable UUID customerId,
                                                  @PathVariable UUID contactId) {
    ...
    public ResponseEntity<Void> removeContact(@PathVariable UUID customerId,
                                              @PathVariable UUID contactId) {
