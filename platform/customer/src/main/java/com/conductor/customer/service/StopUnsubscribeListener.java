...
/**
 * StopUnsubscribeListener — Opt-Out keyword handler (WA-C1).
 * Subscribes to NATS JetStream `message.inbound` events.
 * If keyword is "STOP", revokes marketing consent in under 5 seconds.
 */
@Component
public class StopUnsubscribeListener implements SmartInitializingSingleton {
    ...
    private void handleInboundMessage(ConductorEvent<InboundMessagePayload> event) {
        ...
        try {
            ...
            if (customerId == null) {
                log.warn("Could not resolve customer for phone: {}. Cannot record opt-out.", payload.getFromPhone());
                return;
            }
            ...
            try {
                ...
                log.info("Opt-out already recorded or inactive for customer ID: {}. Detail: {}", customerId, e.getMessage());
            }
    ...
    public static class InboundMessagePayload {
        ...
        public InboundMessageContent getContent() { return content; }
        public void setContent(InboundMessageContent content) { this.content = content; }
