 ...
 *
 * Validation:
 *   - Email: RFC 5322 simple regex check
 *   - Phone: E.164 format check (leading +, 7-15 digits)
 *
 * Identity resolution hook: when a new contact is added, its hash is registered
 * in customer_identifiers for future O(1) duplicate detection.
 *
 * Primary contact rules: at most one contact per type may be isPrimary = true.
...
public class ContactService {
    ...
    @Transactional
    public CustomerContact addContact(UUID customerId, ContactType type, String value,
                                      String label, boolean setPrimary) {
        customerService.requireCustomer(customerId);
        validate(type, value);
        ...
        timelineService.record(customerId, TimelineEventType.CONTACT_ADDED,
                "customer-service",
                "Contact added: " + type.name(),
                String.format("{\"type\":\"%s\",\"label\":\"%s\"}", type, label));

        auditLogger.logEvent("ADD_CONTACT", "CUSTOMER:" + customerId, "SUCCESS",
                "Contact type=" + type + " added");
        return saved;
    }

    @Transactional
    public CustomerContact updateContact(UUID customerId, UUID contactId, String label, boolean setPrimary) {
    ...
    public void setPrimaryContact(UUID customerId, UUID contactId) {
        ...
        auditLogger.logEvent("SET_PRIMARY_CONTACT", "CUSTOMER:" + customerId, "SUCCESS",
    ...
    public void removeContact(UUID customerId, UUID contactId) {
        ...
        timelineService.record(customerId, TimelineEventType.CONTACT_REMOVED,
                "customer-service", "Contact removed: " + contact.getType().name(), null);

        auditLogger.logEvent("REMOVE_CONTACT", "CUSTOMER:" + customerId, "SUCCESS",
    ...
    private void validate(ContactType type, String value) {
        ...
        switch (type) {
            ...
            default -> { /* ADDRESS, SOCIAL, CUSTOM — no structural validation */ }
        }
    }
    ...
    private void registerIdentifier(UUID customerId, ContactType type, String hash) {
        String identifierType = switch (type) {
            case EMAIL    -> "EMAIL";
            case PHONE    -> "PHONE";
